...
const ReportButton: React.FC = () => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const mformat = (m: string) => m.replace(/\d{4}-(\d{2})/, '$1');
      const getvalues = (series: Record<string, number>) => {
        const arr = generateDataByMonth(series, Date.now());
        const map = new Map(arr);
        return months.map((m: string) => map.get(m) ?? 0);
      };

      // === helpers must appear here, before any usage ===
      const lastN = (arr: any[], n: number) => (Array.isArray(arr) ? arr.slice(Math.max(0, arr.length - n)) : []);
      ...
      const lastTwoByMonth = (raw: any) => {
        ...
        return { cur, prev, diff: cur - prev, pct: pct(prev, cur), curMonth, prevMonth };
      };
      // --- end helpers ---
      // 调试日志
      console.log('[调试] normalizeSeries(activity):', normalizeSeries(activity));
      console.log('[调试] normalizeSeries(openrank):', normalizeSeries(openrank));
      console.log('[调试] normalizeSeries(stars):', normalizeSeries(stars));
      console.log('[调试] normalizeSeries(forks):', normalizeSeries(forks));
      console.log('[调试] normalizeSeries(contributor):', normalizeSeries(contributor));
      ...
      // 使用 /api/analyze 生成长文洞察（如已配置 API Key）
      ...
      // helpers
      const lastMonth = lastMonthFrom(normalizeSeries(openrank) || normalizeSeries(activity));

      const starsM = lastTwoByMonth(stars);
      const forksM = lastTwoByMonth(forks);
      const activityM = lastTwoByMonth(activity);
      const openrankM = lastTwoByMonth(openrank);
      const contribM = lastTwoByMonth(contributor);

      const prOpenM = lastTwoByMonth(prOpened);
      const prMergedM = lastTwoByMonth(prMerged);
      const prMergeRateCur = prOpenM.cur > 0 ? Math.round((prMergedM.cur / prOpenM.cur) * 100) : 0;
      const prOpenPrev = prOpenM.prev > 0 ? prOpenM.prev : 1;
      const prMergeRatePrev = Math.round(((prMergedM.prev || 0) / prOpenPrev) * 100);
      const prMergeRateDiff = prMergeRateCur - prMergeRatePrev;

      const issueOpenM = lastTwoByMonth(issuesOpened);
      const issueClosedM = lastTwoByMonth(issuesClosed);
      const issueCommentsM = lastTwoByMonth(issueComments);
      ...
      const period = lastMonth ? `${lastMonth}-01 ～ ${lastMonth}-30` : '';
      const analyzeDate = new Date().toISOString().slice(0, 10);
      const nextQ = lastMonth
        ? (() => {
      ...
      const generateTrendsChartBase64 = async (
        ...
        monthlyEntries: (obj: any) => [string, number][]
      ): Promise<string> => {
        const echarts = await import('echarts');
        ...
        // 提取“最后6个月的真实月份key”
        const activitySeries = normalizeSeries(activity);
        const trend6 = generateDataByMonth(activitySeries, Date.now());
        const months = trend6.map(([month]: [string, number]) => month);
        const mformat = (m: string) => m.replace(/\d{4}-(\d{2})/, '$1');
        const getvalues = (series: Record<string, number>) => {
          const arr = generateDataByMonth(normalizeSeries(series), Date.now());
          // 保证取值顺序和months一致
          const map = new Map(arr);
          return months.map((m: string) => map.get(m) ?? 0);
        };
        ...
        chart.setOption({
          ...
          xAxis: { type: 'category', data: months.map(mformat) },
          yAxis: { type: 'value', splitNumber: 4 },
          series: [
        ...
        await new Promise((res: (value?: unknown) => void) => setTimeout(res, 350));
      ...
      if (chartRef.current) {
        ...
        const months6 = last6(months.map(mformat));
        const activity6 = getvalues(activity).slice(0, 6);
        const openrank6 = getvalues(openrank).slice(0, 6);
        const star6 = getvalues(stars).slice(0, 6);
        const fork6 = getvalues(forks).slice(0, 6);
        const contributor6 = last6(getvalues(contributor));
        ...
        chart.setOption({
          ...
          grid: { top: 65, left: 60, right: 18, bottom: 40 },
          tooltip: { trigger: 'axis', valueFormatter: (v: number) => v.toString() },
          xAxis: { type: 'category', data: months6 },
          yAxis: { type: 'value', splitNumber: 4 },
          series: [
      ...
      let x = generateTrendsChartBase64(activity, openrank, stars, forks, contributor, monthlyEntries);
      console.log(x);
      const md = `# 🗓️ OpenDigger 项目月度报告（${lastMonth ? lastMonth.replace('-', '年') + '月' : ''})
...
| 指标名称 | 当前值 | 环比变化 | 解读 |
|:----------|:--------:|:-----------:|:------|
| **活跃度（Activity）** | ${Math.round(activityM.cur)} | ${pctWord(activityM.pct)} | 开发活跃度${trendWord(activityM.pct)} |
| **影响力（OpenRank）** | ${Math.round(openrankM.cur * 10) / 10} | ${pctWord(openrankM.pct)} | 外部引用与关注度${trendWord(openrankM.pct)} |
| **Star 数** | ${starsM.cur} | ${arrow(starsM.pct)} ${starsM.pct > 0 ? '+' : ''}${starsM.pct}%（${starsM.diff >= 0 ? '+' : ''}${starsM.diff}） | 热度${starDeltaDesc} |
| **Fork 数** | ${forksM.cur} | ${pctWord(forksM.pct)} | 开发者二次利用率${trendWord(forksM.pct)} |
| **贡献者数** | ${contribM.cur} | ${pctWord(contribM.pct)} | 团队规模${trendWord(contribM.pct)} |
...
**亮点**  
- Star 数环比增长 ${plusPct(starsM.pct)}，说明项目近期曝光度提升，可能与版本发布、文档更新或社交传播相关。  
- PR 合并率维持高水平（${prMergeRateCur}%），显示团队协作良好、代码合入高效。
